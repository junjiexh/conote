...
var WS_BASE = (__ENV.WS_BASE || "ws://localhost:8000/collab").replace(/\/$/, "");
var DOC_ID = __ENV.DOC_ID || "711f3de2-d229-4c95-b072-fb275ec17bb4";
var EDIT_INTERVAL_MS = parseInt(__ENV.EDIT_INTERVAL_MS || "1500", 10);
var SESSION_MS = parseInt(__ENV.SESSION_MS || "60000", 10);
var STAGES = __ENV.STAGES ? JSON.parse(__ENV.STAGES) : [
...
var connectionsOk = new Counter("connections_ok");
var connectionsFailed = new Counter("connections_failed");
var connectionsClosed = new Counter("connections_closed");
var editsSent = new Counter("edits_sent");
var messagesReceived = new Counter("messages_received");
var syncErrors = new Counter("sync_errors");
var sendBinary = (socket, u8) => {
  const payload = u8.buffer.slice(u8.byteOffset, u8.byteOffset + u8.byteLength);
  socket.send(payload);
};
var handleSyncMessage = (socket, doc2, decoder) => {
  const encoder = createEncoder();
  writeVarUint(encoder, messageSync);
  try {
    readSyncMessage(decoder, encoder, doc2, null);
  } catch (err) {
    syncErrors.add(1);
  }
  if (length(encoder) > 1) {
    sendBinary(socket, toUint8Array(encoder));
  }
};
var handleAwarenessMessage = (socket, decoder, awareness) => {
  const update = readVarUint8Array(decoder);
  applyAwarenessUpdate(awareness, update, socket);
};
var registerAwareness = (socket, awareness) => {
...
var registerDocUpdate = (socket, doc2) => {
...
var scheduleEdits = (socket, doc2) => {
  let seq = 0;
  const text2 = doc2.getText("content");
  const sendEdit = () => {
    const id2 = `vu${__VU}-t${Date.now()}-${seq++}`;
    doc2.transact(() => {
      const pos = Math.max(0, Math.min(text2.length, Math.floor(Math.random() * (text2.length + 1))));
      text2.insert(pos, ` ${id2}`);
    }, { type: "local-edit", id: id2 });
  };
  socket.setInterval(() => {
...
export {
  collab_ws_k6_default as default,
  options
};
